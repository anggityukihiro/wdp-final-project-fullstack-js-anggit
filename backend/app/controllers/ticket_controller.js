...
export const get_ticket = async (req, res, next) => {
  try {
    ...
    let ticket = await Ticket.findOne({ user_id: user_id });
...
export const add_ticket = async (req, res, next) => {
  try {
    ...
    if (event_id == '' || payment_method_id == '') {
      return res.status(401).json({ message: "Event & Payment Method wajib di isi !" });
    }

    else {
      let get_event = await Event.findOne({ _id: event_id });
      let get_payment_method = await PaymentMethod.findOne({ _id: payment_method_id });
      let get_voucher = await Voucher.findOne({ voucher_code: voucher_id });
      ...
      if (get_event == null) {
        ...
        let total_nominal = get_event._doc.price - get_voucher._doc.cut_price;
        let order_date = new Date();
    
        let ticket = await Ticket.create({  
          user_id: user_id,
          voucher_id: get_voucher._doc._id,
          event_id: event_id,
